...
- **#7 Animate score rings + numbers on reveal** — DONE: `animateRing` helper (sets `strokeDasharray` + animates `stroke-dashoffset` 0->target via a forced reflow + the existing 0.6s CSS transition; reduced-motion CSS makes it instant); `animateNumber` now respects `prefers-reduced-motion`. Applied to Fingerprint + AdBlock rings/numbers — also fixes AdBlock's missing `strokeDasharray` (its ring never filled proportionally before). tsc/eslint 0-warn/283 tests/build green.
- **#4 Explainable metrics (first increment)** — DONE: added `data-tooltip` 'What/why' explanations to the 4 Quality factors (TLS / server RTT / connection type / stability) via the existing tooltip system; `quality.*Factor.tip` x6 locales. Establishes the reusable `data-tooltip` pattern; extending to DNS/Headers/TLS/Fingerprint metrics is incremental. tsc/eslint 0-warn/283 tests/build green.
- **#2 Dashboard 'Run all tests' (sequenced)** — DONE: the 'Run all tests' button already existed but its handler was broken (just navigated to DNS). `runAllTests()` now sequentially runs the button-based tests (Speed -> Headers -> Fingerprint -> Quality) — clicking each start button + waiting for completion via state polling (skip-if-already-done, 90s timeout/test), with a progress label (`dashboard.running` x6 locales). DNS/AdBlock/TLS auto-run on load. **Follow-up DONE**: wired `markCompleted` for headers/fingerprint/quality + added real `computeOverallScore` cases (headers grade->score, fingerprint `100-uniqueness`, quality grade->score), so the overall score now reflects all 7 tests (tls/adblock now real-scored: TLS protocol->grade, adblock score; zero placeholders). tsc/eslint 0-warn/283 tests/build green.