...
async function runQualityTest(): Promise<void> {
  ...
  // Set up stability test button (remove any old listener first)
  const stabilityBtn = document.getElementById("quality-stability-btn") as HTMLButtonElement;
  const progressEl = document.getElementById("stability-progress") as HTMLElement;
  if (stabilityBtn) {
    stabilityBtn.disabled = false;
    // Avoid duplicate listeners
    if (stabilityListenerCleanup) {
    ...
    const handler = async () => {
      ...
      const stability = await ConnectionQuality.runStabilityTest((sent) => {
        if (progressEl) progressEl.textContent = `${sent}/30`;
      });
...
function renderConnectionInfo(info: ConnectionInfo | null): void {
  ...
  el.innerHTML = `
    <div class="info-row">
      <span class="info-label">${t("quality.connType")}</span>
      <span class="info-value">${info.type ? (typeMap[info.type] || info.type) : "—"}</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.effectiveType")}</span>
      <span class="info-value">${info.effectiveType ? info.effectiveType.toUpperCase() : "—"}</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.downlink")}</span>
      <span class="info-value">${info.downlinkMbps !== null ? `${info.downlinkMbps} Mbps` : "—"}</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.rttEstimate")}</span>
      <span class="info-value">${info.rttMs !== null ? `${info.rttMs} ms` : "—"}</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.dataSaver")}</span>
      <span class="info-value">${info.dataSaver ? t("quality.enabled") : t("quality.disabled")}</span>
    </div>
  `;
}
...
function renderTlsInfo(info: TlsInfo | null): void {
  ...
  el.innerHTML = `
    <div class="info-row">
      <span class="info-label">${t("quality.tlsVersion")}</span>
      <span class="info-value mono">${info.version || "—"}</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.cipher")}</span>
      <span class="info-value mono">${info.cipher || "—"}</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.httpProtocol")}</span>
      <span class="info-value mono">${info.httpProtocol || "—"}</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.serverRtt")}</span>
      <span class="info-value">${info.serverTcpRtt !== null ? `${info.serverTcpRtt} ms` : "—"}</span>
    </div>
  `;
}
...
function renderStability(stability: StabilityResults): void {
  ...
  el.innerHTML = `
    <div class="info-row">
      <span class="info-label">${t("quality.min")}</span>
      <span class="info-value mono">${stability.min}ms</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.max")}</span>
      <span class="info-value mono">${stability.max}ms</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.mean")}</span>
      <span class="info-value mono">${stability.mean}ms</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.stddev")}</span>
      <span class="info-value mono">${stability.stddev}ms</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.jitter")}</span>
      <span class="info-value mono">${stability.jitter}ms</span>
    </div>
    <div class="info-row">
      <span class="info-label">${t("quality.packetLoss")}</span>
      <span class="info-value mono">${stability.lossPercent}%</span>
    </div>
  `;
}
...
function renderFinalScore(
  ...
  const labelEl = document.getElementById("quality-grade-label");
  if (labelEl) labelEl.textContent = t(qualityGradeLabels[score.label]) || score.label;
