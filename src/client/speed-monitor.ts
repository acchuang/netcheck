...
export const SpeedMonitor = {
  ...
  async start(duration: MonitorDuration, onResult: (r: SpeedTestResults, index: number) => void): Promise<void> {
    if (this.state?.running) return;
    const totalTests = this._totalTests(duration);
    const startedAt = Date.now();
    const abortController = new AbortController();

    this.state = { running: true, duration, testsCompleted: 0, testsTotal: totalTests, startedAt, abortController };
